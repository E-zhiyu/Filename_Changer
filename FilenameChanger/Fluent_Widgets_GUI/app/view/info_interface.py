...
- 通过侧边导航栏回到主界面，点击输入框右边的文件夹按钮选择需要重命名的对象的父级文件夹
- 点击文件夹按钮右边的列表按钮，在弹出的文件列表界面选择需要重命名的文件
- 点击“文件重命名”按钮，确认操作无误后点击弹出的消息框的“确认”按钮，程序将自动对选中的文件进行重命名
  
## 撤销重命名
- 点击主页的“撤销重命名”按钮将文件夹内的文件名回滚至上一次重命名前的状态，同时删除一条重命名历史记录
...
- 点击规则卡片最右边的“更多”按钮
- 点击弹出菜单中的“修改规则”按钮
- 在弹出的规则输入窗口输入修改后的规则参数并点击确定即可完成修改

### 查看规则详情
- 点击规则卡片最右边的”更多“按钮
- 点击弹出菜单中的“规则详情”即可呼出规则详情界面，该界面会显示规则的关键参数信息

## 历史记录
- 前往“历史记录”界面查看重命名的历史记录，点击历史记录卡片右方的按钮可以查看文件名更改的详细信息
- 点击一个历史记录卡片后点击“删除历史记录”按钮，确认操作后将会删除选中的历史记录
- 点击“清空历史记录”按钮将会删除所有历史记录
- 小提示：从v2.1.0开始历史记录将包含重命名日期，若使用的是旧版的历史记录文件则会显示“未知时间”字样

## 连接数据库
- 前往设置界面,点击编辑连接参数的按钮
- 根据提示输入数据库连接参数
- 输入完成后点击确定,连接参数将保存至本地
- 点击数据库模式开关,软件将进行一次数据库连接测试,测试通过后启用数据库模式

## 规则导入和导出
- 导入：点击设置界面规则导入规则卡片的按钮并选择JSON文件，软件将自动验证其内容格式，验证通过后导入并覆盖现有规则
- 导出：点击设置界面规则导出规则卡片的按钮，软件将复制规则文件到指定位置
...
### 新增内容
- 增加“安全扫描”设置项，默认开启以排除隐藏、只读、系统文件（旧版该功能不可设置且强制开启）

### BUG修复
- 修复修改选中的规则后直接点击激活规则会激活未被选中的规则BUG

### 更改内容
- 日志内容不会记录日期，因为与日志文件名重复
- 启用文件夹模式后使用修改扩展名的规则不会执行重命名操作
- 修改文件列表窗口大小

# v2.2.3
该版本为BUG修复和优化的版本，无新增特性

### 优化和修复
- 修复添加第一个规则时规则卡片显示出错的BUG
- 修复文件列表点击文件卡片崩溃的BUG
- 修复规则输入窗口从第五类规则切换到其他规则后显示错误的BUG
- 修复规则为空时开始重命名会崩溃的BUG
- 现在主页清空目标文件夹路径后不会弹出消息条
...
### 优化和修复
- 修复应用主题设置为“跟随系统”后重启应用显示不是“跟随系统”的BUG
- 修复软件启动后直接点击文件列表按钮导致崩溃的BUG
- 完善规则导入时文件内容的验证流程
- 现在重命名结束后会分别显示成功和失败的文件个数
- 主界面路径输入框的路径改变后会通过消息条显示路径是否有效
- 优化规则列表和历史记录列表的删除算法，减小性能开销
- 优化规则列表的添加规则和修改规则的算法
- 优化主界面执行重命名以及撤销重命名后更新历史记录的算法
- 历史记录界面删除历史记录不会强制滚动到顶部
- 现在文件列表为空时会显示“文件夹为空”

# v2.2.1

### 新增内容
- 第一类规则（交换分隔符前后内容）的分隔符支持正则表达式
- 增加文件列表界面，用户可以在该界面选择需要重命名的文件
- 第五类规则（文件编号）可选是否重命名为自定义名称再编号
- 新增软件设置界面
- 增加规则导入和导出功能（设置界面）
- 增加第七类规则：添加字符串
...
- 修复文件重命名失败后历史记录增加但是不显示在历史记录界面的BUG
- 修复使用第四类规则会误移除文件名中长串数字的BUG
...
- 为一些操作增加界面顶部的提示弹窗
- 规则描述为空时日志记录为“描述：<空>”
- 优化规则卡片和历史记录卡片被选中时文本标签和图标的辨识度

### 更改内容
- 第四类规则的日期输入控件改为日期选择器
- 将一些操作完成后的模态弹窗改为界面顶部消息框
- 规则列表界面激活规则后会重置卡片的选中状态
- 第四类规则更名为“日期填充”
- 第三类规则启用正则时移除新字符串输入框的输入限制器
