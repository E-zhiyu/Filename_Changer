...
## 撤销重命名
- 点击主页的“撤销重命名”按钮将文件夹内的文件名恢复到上一次重命名前的状态，同时删除一条重命名历史记录，此操作不需要依赖任何重命名规则
...
## 规则导入和导出
- 点击设置界面规则导入规则卡片的按钮并选择JSON文件，软件将自动验证其内容格式，验证通过后导入并覆盖现有规则
- 点击设置界面规则导出规则卡片的按钮，软件将复制规则文件到指定位置

# 注意事项
- 为了确保文件重命名不会损害您的设备，本程序将不会修改任何隐藏文件、只读文件和系统文件，尽管如此，我仍建议您将需要重命名的文件单独存放至一个专门的文件夹内，以免造成不可挽回的后果。
"""
